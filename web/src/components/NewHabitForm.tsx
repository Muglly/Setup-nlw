...
export function NewHabitForm() {
  ...
  const handleToggleWeekDay = (weekDayIndex: number) => {
    if (weekDays.includes(weekDayIndex)) {
      setWeekDays((preveState) =>
        preveState.filter((weekDays) => weekDays !== weekDayIndex)
      );
    } else {
  ...
  return (
    <form onSubmit={createNewHabit} className="w-full flex flex-col mt-6">
      ...
      <input
        type="text"
        id="title"
        placeholder="ex.: Beber água, estudar, etc..."
      ...
      <button
        type="submit"
        className="mt-6 rounded-lg p-4 flex items-center justify-center gap-3 font-semibold bg-green-600 hover:bg-green-500 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 focus:ring-offset-zinc-900"
      >
